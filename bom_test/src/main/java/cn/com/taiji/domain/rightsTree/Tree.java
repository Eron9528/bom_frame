...
public class Tree<T> {
     ...
     * 平铺树节点,id与树节点关系
     */
    private Map<Object, TreeNode<T>> idMapTree = new TreeMap<>();

    /**
     * 平铺树节点,data与树节点关系
     */
    private Map<T, TreeNode<T>> tMapTree = new TreeMap<T, TreeNode<T>>(
            //比较器按照降序实现
            (Comparator<? super T>) new Comparator<Permission>(){
                public int compare(Permission order1, Permission order2) {
                    return order2.getMenu().compareTo(order1.getMenu());
                }
            });
